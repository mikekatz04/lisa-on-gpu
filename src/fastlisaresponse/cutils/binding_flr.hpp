...
class CubicSplineWrap_responselisa {
  public:
    CubicSpline *spline;
...
class TDIConfigWrap{
  public:
    ...
    TDIConfigWrap(array_type<int>unit_starts_, array_type<int>unit_lengths_, array_type<int>tdi_base_link_, array_type<int>tdi_link_combinations_, array_type<double>tdi_signs_in_, array_type<int>channels_, int num_units_, int num_channels_)
    {
        ...
        int *_tdi_base_link = return_pointer(tdi_base_link_);
        int *_tdi_link_combinations = return_pointer(tdi_link_combinations_);
        double *_tdi_signs_in = return_pointer(tdi_signs_in_);
        int *_channels = return_pointer(channels_);
    ...
    template<typename T>
    T* return_pointer(array_type<T> input1)
    {
#if defined(__CUDA_COMPILATION__) || defined(__CUDACC__)
        T *ptr1 = static_cast<T *>(input1.get_compatible_typed_pointer());
